...
#else
PNG_FUNCTION(void, png_err,(png_structp png_ptr),PNG_NORETURN)
{
   if (png_ptr != NULL && png_ptr->error_fn != NULL)
...
void png_benign_error(png_structp png_ptr, png_const_charp error_message)
{
...
#ifdef PNG_BENIGN_ERRORS_SUPPORTED
void png_chunk_benign_error(png_structp png_ptr, png_const_charp error_message)
{
   if (png_ptr->flags & PNG_FLAG_BENIGN_ERRORS_WARN)
...
void png_fixed_error(png_structp png_ptr, png_const_charp name)
{
 ...
 */
jmp_buf * png_set_longjmp_fn(png_structp png_ptr, png_longjmp_ptr longjmp_fn,
    size_t jmp_buf_size)
{
