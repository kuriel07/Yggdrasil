...
#define STATIC
STATIC void __RdPage512(uint8 *buf)
{
	unsigned i;
	for (i = 0; i < 512; i++) {
		buf[i] =  NF_RDDATA8();
...
STATIC uint32 NF8_CheckId(void)
{
	int i;
	uint8 Mid, Did, DontCare, id4th;  		
...
STATIC int Nand_IsBadBlockPage512(uint32 block)
{
    uint32 	Page;
	uint32 		BAD;
...
STATIC int Nand_IsBadBlockPage2048(uint32 block)
{
    uint32 	Page;
	uint8 		BAD;
...
STATIC int Nand_IsBadBlock(uint32 block,int bLargeBlock)
{
	switch(bLargeBlock) {
	case 0:
...
STATIC int Nand_ReadSectorPage512(uint32 sector, uint8 *buffer)
{
...
STATIC int Nand_ReadSectorPage2048(uint32 sector, uint8 *buffer)
{
	uint32 page, data_addr;
...
STATIC int Nand_ReadSector(uint32 sector, uint8 *buffer, int bLargeBlock)
{
	switch(bLargeBlock) {
	case 0:
