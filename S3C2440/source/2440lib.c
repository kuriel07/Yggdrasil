...
//********************** BOARD LCD backlight ]****************************
void LcdBkLtSet(uint32 HiRatio)
{
...
uint16 GetCoreClock(void)
{
    uint16 mdiv = (uchar)(rUPLLCON >> 12) & 0xff;
    uint16 pdiv = (uchar)((rUPLLCON >> 4) & 0x3f);
    uint16 sdiv = (uchar)(rUPLLCON & 0x03);
    mdiv += 8;
    pdiv += 2;
    return (uint16)(2 * mdiv * 12) / (pdiv * (1<<sdiv));
}

//*************************[ General Library ]**********************
#if !USE_MAIN
void * c_malloc(unsigned nbyte) 
